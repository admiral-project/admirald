...
- Publish tasks to RabbitMQ for `admiral-fleet` workers
...
export ADMIRAL_RABBITMQ_URL=amqps://guest:guest@localhost:5671/admiral
...
`admirald` does not execute containers directly. It delegates all local execution to `admiral-fleet` agents via RabbitMQ task queues.
...
- RabbitMQ 3 with TLS for async task dispatch
